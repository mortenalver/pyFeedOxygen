...
advectFeed = False # If false, feed advection will be skipped
useNetcdfInputs = True # Read and update input data from NetCDF file (OF1)
perturbations = not dryRun # not dryRun # Add random perturbations to process (needed for EnKF):
...
# for k in range(0, cageDims[2]):
#     lessThanMaxDepth = (float(k) - 0.5) * dz < fishMaxDepth
#     for i in range(0, cageDims[0]):
#         for j in range(0, cageDims[1]):
#
#             if mask[i, j, k] and lessThanMaxDepth:
#                 o2Affinity[i, j, k] = 1
#
...
storage.createCageVariables(ncfile, ('feed', 'o2', 'ingDist', 'o2consDist'))
...
for i in range(0, n_steps):
    ...
    if useNetcdfInputs:
        ...
        if i==0 or isNewData:
            print("Updating environment, t="+str(t))
            inpDepths = [5., 10., 15.]
            # Temperature:
            ...
            diffKappaO2 = min(0.5, 10 * math.pow(currentReductionFactor*0.04, 2))
            diffKappaO2Z = 0.1*diffKappaO2
    ...
    # Advect fields:
    if advectFeed:
        lostAll = fcAdvect.advectField(cageDims, fc, dt, dxy, dz, mask, sinkingSpeed_r, diffKappa, diffKappaZ,
    ...
    if i>0 and (math.floor(t % storeIntervalFields)) < dt:
        storage.saveCageVariable(ncfile, t, 'feed', fc, True, mask, True)
        storage.saveCageVariable(ncfile, t, 'o2', o2, True, mask, False)
    ...
    if i>0 and i % txtUpdateInterval == 0:
